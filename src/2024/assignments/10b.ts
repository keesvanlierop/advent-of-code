...
const run = async () => {
	...
	const detectValidTrailHeads = (startX: number, startY: number) => {
		const iterate = (x = startX, y = startY, current = 0, path: string = '') => {
			...
			if (level === 9) {
				...
				validTrailheads[`${startX}-${startY}`].add(path)
				return
			}
			...
			directions.forEach(([dx, dy]) => {
				...
				const nextY = y + dy
				path += `-${nextX}-${nextY}`
				iterate(nextX, nextY, level, path)
			})
		}
	...
	console.log(`The answer for 10b is: ${add(Object.values(validTrailheads).map((trailheads) => trailheads.size))}`)
}
