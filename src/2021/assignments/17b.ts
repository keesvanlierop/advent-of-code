...
const getValidProbes = (targetArea: ReturnType<typeof getTargetArea>) => {
	...
	for (let xVelocity = 0; xVelocity < targetArea.x.max * 2; xVelocity++) {
		for (let yVelocity = 100; yVelocity > targetArea.y.min * 2; yVelocity--) {
...
const run = async () => {
	...
	console.log(validProbes.length)

	console.log(`The answer for 17b is: ${validProbes.length}`)
}
