...
const getFoldState = (list: string[]) => {
	...
	const firstInstruction = [state.instructions[0]]
	firstInstruction.forEach(([direction, position]) => {
		if (direction === 'y') {
...
const run = async () => {
	...
	const amountOfDots = Object.values(state.map.coordinatesX).flat().length

	console.log(`The answer for 13a is: ${amountOfDots}`)
}
