...
const getVentsDiagram = (list: string[]) => {
	...
	list.map((item) => {
		...
		if (isOrtagonal) {
			...
			for (let x = startX; x <= endX; x++) {
				if (!diagram[x]) diagram[x] = []
				const startY = startX === x1 ? y1 : y2
				const modifier = (startY === y1 && y1 > y2) || (startY === y2 && y2 > y1) ? -1 : 1
				const yIncrement = x - startX
				const y = startY + yIncrement * modifier
				diagram[x].push(y)
			}
		}
	})
...
const run = async () => {
	...
	console.log(`The answer for 5b is: ${points}`)
}
