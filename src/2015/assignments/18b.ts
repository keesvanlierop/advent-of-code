...
const getLightsState = (map: string[]) => {
	...
	state[`0,0`] = true
	state[`0,${map.length - 1}`] = true
	state[`${map[0].length - 1},${map.length - 1}`] = true
	state[`${map[0].length - 1},0`] = true
...
const animate = (state: typeof defaultState, map: string[]) => {
	...
	cachedState[`0,0`] = true
	cachedState[`0,${map.length - 1}`] = true
	cachedState[`${map[0].length - 1},${map.length - 1}`] = true
	cachedState[`${map[0].length - 1},0`] = true
...
const run = async () => {
	...
	console.log(`The answer for 18b is: ${Object.keys(lightsState).length}`)
}
