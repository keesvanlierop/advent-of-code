...
const getStacksAfterProcedures = (stacks: string[][], procedures: Procedure[]) => {
    procedures.forEach(procedure => {
        const stack = stacks[procedure.from - 1]
        const moved = stack.splice(stack.length - procedure.move, procedure.move)
        moved.forEach(crate => {
...
const run = async () => {
	...
	console.log(
		`The answer for 5b is: ${[...stacksAfterProcedures].map(stack => stack.pop()).join('')}`,
	)
}
