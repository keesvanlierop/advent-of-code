export const getNextSeparatorIndex = <T extends any, Separator extends T>(
	array: T[],
	separator: Separator,
	currentIndex: number,
) => {
...
export const chunkBy = <T extends any, Separator extends T>(
	array: T[],
	separator: Separator,
) => {
	const chunkedMap: Record<string, T[]> = array.reduce((map, item, index) => {
		const nextSeparatorIndex = getNextSeparatorIndex(
			array,
			separator,
			index,
		)
